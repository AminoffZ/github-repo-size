...
<div>
  <div>
    <h1
      class="select-none whitespace-nowrap px-8 py-4 text-ctp-pink text-3xl font-black"
    >
      GitHub Repo Size
    </h1>
  </div>
  ...
  >
    ...
    <div class="flex justify-center items-center">
      <button
        class="bg-ctp-crust"
        on:click={() => (tokenEnabled = !tokenEnabled)}>Toggle Token</button
      >
    </div>
  </div>
  <div class="flex justify-center">
    <button
      class="flex justify-center w-full hover:bg-ctp-crust"
      on:click={() => (expanded = !expanded)}
    >
      <svg
        class="arrow {expanded
          ? 'arrow-up'
          : 'arrow-down'} h-10 w-10 fill-ctp-text"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
